...
public class BookOfferGUI extends JDialog {
	...
	public BookOfferGUI(Offer offer, Usuario user, GUIOperator operator, boolean verInfo, boolean esPropietario) {
		...
		try
		{
			img = new JLabel (operator.resizeIcon(new ImageIcon(operator.decodeToImage(rh.getImagenes().get(0))), 450, 450));
			...
			//IMAGEN POR DEFECTO
		JLabel img = new JLabel(operator.resizeIcon(new ImageIcon(ImageUtils.decodeToImage(ImageTypes.DefaultHouse)), 450, 450));
		img.setBounds(67, 152, 450, 450);
		contenpane.add(img);
		...
		anterior.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				img = new JLabel (operator.resizeIcon(new ImageIcon(operator.decodeToImage(rh.getImagenes().get(ifotos))), 450, 450));
				img.setBounds(67, 152, 450, 450);
				contenpane.add(img);
				img.repaint();
				contenpane.repaint();
				...
				if (ifotos==0) anterior.setEnabled(false);
		...
		siguiente.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (ifotos+1==rh.getImagenes().size()) siguiente.setEnabled(false);
				anterior.setEnabled(true);
			}
		});
		siguiente.setBounds(353, 612, 164, 42);
		if (rh.getImagenes()!=null){
			if (rh.getImagenes().size()>1) siguiente.setEnabled(true);
		...
		contacto.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!esPropietario){
					RedactarMsgGUI.main(null, user ,rh.getOwner().getCorreo(),"Información sobre la casa en:" + rh.getCity(),"", operator);
				}
				
				else RedactarMsgGUI.main(null, user ,offer.getCliente().getCorreo(),"Información sobre tu reserva: ","", operator);
			}
		});
		...
		JButton reservat = new JButton("Reservar");
		reservat.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (user.equals("anonymus")){
					Component frame=null;
					JOptionPane.showMessageDialog(frame,
							"No puedes realizar reservas en modo anónimo, registrate o inicia sesión",
							"Identificate para acceder a esta función", JOptionPane.WARNING_MESSAGE);
		...
		reservat.setBounds(723, 662, 359, 55);
		contenpane.add(reservat);
		}
		...
		JLabel lblNombreDelDueo = new JLabel("Nombre del due\u00F1o: " + ow.getNombre() + " "+ ow.getApellido());
		...
		contenpane.add(lblTelfono);
			...
			//TODO LO DEL CATCH SE PUEDE BORRAR CUANDO ACTUALICEMOS TODAS LAS CASAS DE LA BASE DATOS
