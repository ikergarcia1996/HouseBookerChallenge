...
public class AyudaGUI extends JDialog {
	...
	public AyudaGUI(GUIOperator operator) {
		...
		JLabel lblSiNecesitasAyuda = new JLabel("Si necesitas ayuda con nuestra aplicacion puedes contactar con el administrador:");
		...
		btnYaHeContactado.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String uuid = JOptionPane.showInputDialog(null, "Introduzca aqui el identificador que se le dio cuando realizó su consulta.","Identificador",JOptionPane.INFORMATION_MESSAGE);
				Mensaje mensaje=operator.searchPublic(uuid);
				
				if (mensaje==null||!Pattern.matches("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", uuid)) JOptionPane.showMessageDialog(null, "No se ha encontrado ningun mensaje con ese identificador.", "Error",
						JOptionPane.WARNING_MESSAGE);
				else ViewPublicMsgGUI.main(mensaje);
