...
public class DB4oManagerServer extends JDialog {
	...
	public DB4oManagerServer() {
		...
		if (c.isDatabaseLocal()) {
			...
			try {
				...
				server = Db4oClientServer.openServer(configurationCS, c.getDb4oFilename(), c.getDatabasePort());

				textArea.append("\nConnection to the database '" + c.getDb4oFilename() + "' opened in port "
						+ c.getDatabasePort());
	...
	public void initializeDB() {
		...
		configurationCS.common().updateDepth(c.getUpdateDepth());
		configurationCS.common().objectClass(RuralHouse.class).cascadeOnDelete(true);
		ObjectContainer db = Db4oClientServer.openClient(configurationCS, c.getDatabaseNode(), c.getDatabasePort(),
				c.getUser(), c.getPassword());

	//	RuralHouse rh1 = new RuralHouse(1, "Ezkioko etxea", "Ezkio", null);
	//	RuralHouse rh2 = new RuralHouse(2, "Etxetxikia", "Iru�a", null);
